...
module.exports = function unMarshalBinaryLengthPrefixed(bytes, type) {
  ...
  if (len < 0) throw new Error(`Error reading msg byte-length prefix: got code ${len}`);
...
const decodeBinary = (bytes, type, isLengthPrefixed) => {
  if (type === 'tendermint/PubKeySecp256k1') {
    const { val, offset } = decoder(bytes, varBytes);
    return {
      val: amino.unmarshalPubKeySecp256k1(val, false),
      offset,
    };
  }
  ...
  keys.forEach((key, index) => {
    ...
    if (is.array(type[key])) {
      ...
      if (fieldNum <= lastFieldNum) {
        // When sequence is zero, somehow it's missing from the tx data
        if (key === 'sequence') {
          return;
        }
      ...
      if (typ !== typeWanted) {
        throw new Error(
          `Field type is not expected: key=${key} wanted=${typeWanted} actual=${typ} fieldNum=${fieldNum} len=${fieldNumLen}`
        );
      }
